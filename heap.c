...
static void set(Heap *h, size_t k, void *x) {
    h->data[k] = x;
    h->setpos(x, k);
}

static void swap(Heap *h, size_t a, size_t b) {
    void *tmp;
...
static int less(Heap *h, size_t a, size_t b) {
...
static void siftup(Heap *h, size_t k) {
    for (;;) {
        ...
        size_t s = k;
        if (l < h->len && less(h, l, s))
            s = l;
        if (r < h->len && less(h, r, s))
            s = r;
