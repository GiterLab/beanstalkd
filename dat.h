...
#define __twarn(fmt, ...) warn("%s:%d in %s: " fmt "%s", __FILE__, __LINE__, __func__, __VA_ARGS__)
...
// See __twarn macro.
#define __twarnx(fmt, ...) warnx("%s:%d in %s: " fmt "%s", __FILE__, __LINE__, __func__, __VA_ARGS__)
...
Job *make_job_with_id(uint pri, int64 delay, int64 ttr, int body_size, Tube *tube, uint64 id);
void job_free(Job *j);
